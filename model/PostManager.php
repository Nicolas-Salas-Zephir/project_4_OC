...
class PostManager extends Manager
{
    ...
    {
        $db = $this->dbConnect();
        $req = $db->query('SELECT id, title, content, DATE_FORMAT(create_date, \'%d/%m/%Y à %Hh%imin%ss\') AS create_date_fr FROM posts ORDER BY create_date DESC LIMIT 0, 3');
    ...
    {
        $db = $this->dbConnect();
        $req = $db->query('SELECT id, title, content, DATE_FORMAT(create_date, \'%d/%m/%Y à %Hh%imin%ss\') AS create_date_fr FROM posts ORDER BY create_date DESC LIMIT 0, 20');
